...
## B13 — 균주쌍 완성 CAC (semantic + 다중-CLIB) on 항원거리 (`phaseB13_pairwise_cac.py`)
B12의 semantic-단독을 완성 CAC로 보강. Smith-2004 항원지도 273균주, 정렬 328위치. 균주쌍 다중-CLIB(t) = Σ 차이위치 log P(modal codon→aa, t) 대칭화. semantic+CLIB 부호 지도적합 → 항원거리, 시간 통제.
